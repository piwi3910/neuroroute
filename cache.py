...
class Cache:
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        self.reconnect_delay = redis_config.get("reconnect_delay", 5)  # seconds
    ...
    def _initialize_connections(self):
        ...
        try:
            ...
            self.async_client = Redis(
                ...
                socket_connect_timeout=self.redis_config.get("connect_timeout", 3)
            )
    ...
    def _generate_key(self, prompt: str, metadata: Dict[str, Any] = None) -> str:
        ...
        filtered_metadata = {k: v for k, v in metadata.items() if k in cache_relevant_fields}
    ...
    async def set(self, prompt: str, response: Dict[str, Any], metadata: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            logger.debug(f"Cached response for key: {key[:10]}... with TTL: {ttl}s")
            return True
    ...
    async def clear(self, model: Optional[str] = None) -> int:
        ...
        try:
            ...
            if model:
                ...
                logger.info(f"Cleared {cleared_count} cached responses for model {model}.")
            else:
                # Clear all cache
                ...
                logger.info(f"Cleared {cleared_count} cached responses.")
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            while True:
                ...
                entry_keys = [k for k in keys if not k.startswith(f"{self.prefix}models:")]
...
def get_cache(settings: Optional[Settings] = None) -> Cache:
    ...
    Args:
        settings: Optional settings to initialize cache with
